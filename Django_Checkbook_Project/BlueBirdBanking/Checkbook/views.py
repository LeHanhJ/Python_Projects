...
def balance(request, pk):
    ...
    current_total = account.initial_deposit  # Create a dictionary into which transaction information will be placed
    ...
    for t in transactions:  # Loop through transactions and determine which is a deposit or a withdrawal
        if t.type == 'Deposit':
            ...
            current_total -= t.amount  # If deposit add amount to balance
            table_contents.update({t: current_total}) # Add transaction total to dictionary
    # Pass account, account total balance, and transaction information to template
...
def transaction(request):
    ...
    if request.method == 'POST':
        if form.is_valid():  # Check to see if the submitted form is valid and if so, saves the form
            pk = request.POST['account'] #Retrieve which account the transaction is for
            form.save()  # Saves the transaction form
            return balance(request, pk) # Renders  balance of the accounts on Balance Sheet
